...
ACCURACY_LOG_PATH = 'accuracy_cnn.txt'
CONFUSION_MATRIX_PATH = 'confusion_matrix_cnn.png'
F1_SCORE_PATH = 'f1_score_cnn.txt'
...
def load_data(csv_file):
    ...
    for index, row in df.iterrows():
        file_path = os.path.normpath(os.path.join(DATASET_PATH, 'Train_submission', row.iloc[0]))
...
X_train = X_train.reshape(-1, 13, 1, 1)  # 13 MFCC, 1 временной шаг, 1 канал
X_test = X_test.reshape(-1, 13, 1, 1)

# Создание модели CNN
model = models.Sequential()
model.add(layers.Input(shape=(13, 1, 1)))  # Используем Input как первый слой
...
# Измените выходной слой
model.add(layers.Flatten())  # Убедитесь, что выход плоский
model.add(layers.Dense(len(np.unique(y_train_encoded)), activation='softmax'))  # Выходной слой
